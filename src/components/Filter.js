...
export const Filter = () => {
	...
	return (	
		...
		<DropdownButton id="dropdown-item-button" title="Filter" disabled={loading}>
			<Dropdown.Item as="button" onClick={() => submitHandler(null)}>all</Dropdown.Item>
			{
				state.map(item => item ? <Dropdown.Item as="button" key={item} onClick={() => submitHandler(item)}>{item}</Dropdown.Item> : null)
			}
			</DropdownButton>
		</Col>
	)
